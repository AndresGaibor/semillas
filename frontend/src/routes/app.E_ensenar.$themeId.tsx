          ...
          <div className="bg-white rounded-[2.5rem] p-8 sm:p-12 shadow-2xl shadow-slate-200/50 border border-slate-100 min-h-[500px] flex flex-col w-full">
                    ...
                    {actividad.tipo_actividad?.codigo === 'cuestionario' && actividad.opciones && (
                      <div className="flex flex-col gap-3 mt-4">
                        {actividad.opciones.map((opcion: any) => (
                          <div key={opcion.id} className="flex items-center gap-3 p-4 rounded-xl bg-white border border-slate-200 hover:border-[#eab308] hover:bg-yellow-50/30 cursor-pointer transition-all">
            ...
            <div className="w-full mt-4 pt-6 border-t border-slate-100 flex flex-col gap-4">
              <Link
                ...
                style={{ backgroundColor: '#eab308', color: '#ffffff', boxShadow: '0 20px 25px -5px rgba(234, 179, 8, 0.3), 0 8px 10px -6px rgba(234, 179, 8, 0.1)' }}
              >
                Siguiente Fase
              </Link>
              <Link
