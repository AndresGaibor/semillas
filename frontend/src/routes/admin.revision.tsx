          ...
          {query.data.revisiones.map((revision) => (
            <article key={revision.id} className="admin-editor-section">
              <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-6">
                <div>
                  <div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-50 text-violet-600"><Clock3 size={19} /></span><div><span className="admin-eyebrow">{revision.senda}</span><h2 className="mt-1 text-xl font-black text-slate-900">{revision.titulo}</h2></div></div>
                  <p className="mt-4 text-sm leading-7 text-slate-600">Enviado por <strong>{revision.enviado_por}</strong>. {revision.notas || "El autor no agregó notas."}</p>
                ...
                <div className="rounded-3xl bg-slate-50 p-4"><label className="admin-field"><span>Notas de revisión</span><textarea rows={4} value={notes[revision.tema_id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [revision.tema_id]: event.target.value }))} placeholder="Explica los cambios solicitados o la razón de la decisión..." /></label><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" disabled={mutation.isPending} className="inline-flex items-center justify-center gap-1 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white" onClick={() => mutation.mutate({ themeId: revision.tema_id, status: "aprobado" })}><CheckCircle2 size={15} /> Aprobar</button><button type="button" disabled={mutation.isPending} className="inline-flex items-center justify-center gap-1 rounded-xl bg-amber-500 px-3 py-2 text-xs font-bold text-white" onClick={() => mutation.mutate({ themeId: revision.tema_id, status: "cambios_solicitados" })}><MessageSquareWarning size={15} /> Cambios</button><button type="button" disabled={mutation.isPending} className="inline-flex items-center justify-center gap-1 rounded-xl bg-red-600 px-3 py-2 text-xs font-bold text-white" onClick={() => mutation.mutate({ themeId: revision.tema_id, status: "rechazado" })}><XCircle size={15} /> Rechazar</button></div></div>
              </div>
            </article>
          ))}
        </div>
