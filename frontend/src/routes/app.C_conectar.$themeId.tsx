...
function CConectarPage() {
  ...
  // El themeId de la URL puede ser un slug, así que primero obtenemos el tema
  ...
  console.log("=== Datos en C_conectar ===");
  console.log("Paso Actual:", pasoActual);
  console.log("Contenido:", contenidoPaso);
  console.log("Actividades de la fase:", actividadesFase);
  console.log("===========================");
      ...
      <div className="w-full px-4 sm:px-8 pt-6 pb-10 flex flex-col gap-6 relative z-10">
        ...
        <div className="w-full h-[200px] sm:h-[280px] rounded-[2.5rem] overflow-hidden relative shadow-lg shadow-slate-200/50">
          ...
          <div className="absolute bottom-4 left-6 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-full shadow-sm border border-white/20">
             <span className="text-sm font-bold text-slate-700 tracking-wide uppercase">Conectar</span>
          </div>
        </div>

        {/* Contenedor condicional para Loading/Error/Contenido */}
        {isLoading ? (
          <div className="flex flex-col gap-4 justify-center items-center h-64 bg-white rounded-[2.5rem] shadow-xl text-[#43a047]">
            <Loader className="animate-spin size-12" />
            <p className="font-bold animate-pulse text-xl">Cargando modelo CRECER...</p>
          </div>
        ) : isError ? (
                    ...
                    {actividad.tipo_actividad?.codigo === 'cuestionario' && actividad.opciones && (
                      <div className="flex flex-col gap-3 mt-4">
                        {actividad.opciones.map((opcion: any) => (
                          <div key={opcion.id} className="flex items-center gap-3 p-4 rounded-xl bg-white border border-slate-200 hover:border-[#16a34a] hover:bg-green-50/30 cursor-pointer transition-all">
