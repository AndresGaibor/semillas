        ...
        <aside className="admin-editor-aside">
          <section className="admin-editor-section">
            ...
            <div className="rounded-3xl bg-slate-50 p-4">
              {editor.selectedResource?.tipo === "imagen" ? <img src={editor.selectedResource.url_publica} alt="" className="mb-4 aspect-video w-full rounded-2xl object-cover" /> : <div className="mb-4 grid aspect-video place-items-center rounded-2xl bg-slate-100 text-slate-300"><ImageIcon /></div>}
              <span className="admin-eyebrow">{editor.activeStep?.nombre ?? "Paso"}</span>
              <h3 className="mt-2 text-xl font-black text-slate-900">{editor.title || "Título pendiente"}</h3>
              <p className="mt-3 line-clamp-6 whitespace-pre-line text-sm leading-7 text-slate-600">{editor.body || "Escribe el contenido para ver una previsualización."}</p>
          ...
          <section className="admin-editor-section">
            <h2 className="font-black text-slate-800">Cobertura de la franja</h2>
