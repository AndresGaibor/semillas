          ...
          {activeTab === "general" ? (
            <section className="admin-editor-section">
              ...
              <div className="admin-form-grid">
                ...
                <Field label="Franjas disponibles" wide><div className="grid grid-cols-3 gap-3">{edadesQuery.data?.map((grupo) => { const selected = form.grupo_edad_ids.includes(grupo.id); return <button key={grupo.id} type="button" onClick={() => updateForm("grupo_edad_ids", selected ? form.grupo_edad_ids.filter((id) => id !== grupo.id) : [...form.grupo_edad_ids, grupo.id])} className={`rounded-2xl border p-4 text-left ${selected ? "border-violet-400 bg-violet-50" : "border-[#2a4a3a] bg-[#142e22]"}`}><span className="flex items-center justify-between font-bold text-emerald-50">{grupo.nombre}{selected ? <CheckCircle2 size={18} className="text-violet-600" /> : <CircleIcon />}</span><small className="mt-1 block text-emerald-400/50">{grupo.edad_minima}–{grupo.edad_maxima} años</small></button>; })}</div></Field>
              </div>
            </section>
          ) : activeTab === "portada" ? (
            <section className="admin-editor-section">
              ...
              <div className="grid grid-cols-[minmax(0,1fr)_280px] gap-6">
                <div className="overflow-hidden rounded-3xl border border-[#2a4a3a] bg-[#1a3a2a]"><img src={cover} alt={`Portada actual de ${tema.titulo}`} className="aspect-[16/9] w-full object-cover" /></div>
                <div className="flex flex-col justify-center gap-3"><button type="button" className="admin-primary-button" disabled={coverMutation.isPending} onClick={() => fileInput.current?.click()}><Upload size={17} /> Subir nueva portada</button><button type="button" className="admin-secondary-button" disabled={!tema.portada_recurso_id || coverMutation.isPending} onClick={() => coverMutation.mutate(null)}><Trash2 size={17} /> Quitar portada</button><p className="text-xs leading-6 text-emerald-300/60">Recomendado: WebP o JPG, relación 16:9, mínimo 1200 × 675 px. El archivo se registra en el módulo de medios.</p></div>
              </div>
            </section>
          ) : (
            <section className="admin-editor-section">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-3xl bg-[#0d1f17] p-5"><span className="admin-eyebrow">Estado actual</span><strong className="mt-3 block text-3xl font-black capitalize text-emerald-50">{tema.estado}</strong><p className="mt-2 text-sm leading-6 text-emerald-300/60">Versión de contenido {tema.version_contenido}. Cada publicación incrementa la versión para las descargas offline.</p></div>
                <div className="rounded-3xl bg-[#0d1f17] p-5"><span className="admin-eyebrow">Completitud</span><strong className="mt-3 block text-3xl font-black text-emerald-50">{completitud?.porcentaje ?? 0}%</strong><p className="mt-2 text-sm leading-6 text-emerald-300/60">{completitud?.listo_para_revision ? "El tema puede entrar al flujo editorial." : "Completa los criterios pendientes antes de enviarlo."}</p></div>
              </div>
...
function CircleIcon() { return <span className="h-4 w-4 rounded-full border-2 border-emerald-400/50" />; }
