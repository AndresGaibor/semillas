...
export function AdminActivitiesTabs({ activeTab, onTabChange, tabCounts }: AdminActivitiesTabsProps) {
  return (
    <div className="flex gap-4 border-b border-slate-200 pb-px mb-2 text-left select-none overflow-x-auto whitespace-nowrap scrollbar-none">
      <Boton
      ...
      >
        ...
        Quiz
        {tabCounts.quiz > 0 && <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-purple-100 text-purple-600 font-bold">{tabCounts.quiz}</span>}
      </Boton>
      <Boton
      ...
      >
        ...
        Flashcards
        {tabCounts.flashcards > 0 && <span className="px-1.5 py-0.5 rounded-full text-[10px] bg-amber-100 text-amber-600 font-bold">{tabCounts.flashcards}</span>}
      </Boton>
      <Boton
      ...
      <Boton
        ...
        variante="texto"
      >
        <i className="fa-solid fa-circle-check text-slate-8000" />
        Verdadero/Falso
      </Boton>
      <Boton
